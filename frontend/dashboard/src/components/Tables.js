...
export const TweetsTable = (props) => {
    const { title, data, clip = 30, seeAll } = props;

    const clipTweet = (text, cutOff) =>
        text.length < cutOff ? `${text.toString().slice(0, cutOff)}...` : text;

    const TableRow = (props) => {
        const { username, text, sentiment, retweet_count } = props;
        const styles = {
            positive: { icon: faArrowUp, color: "text-success" },
            negative: { icon: faArrowDown, color: "text-danger" },
            neutral: { icon: faMinus, color: "text-primary" },
        };
        const sent =
            sentiment === 0
                ? styles.neutral
        ...
        return (
            <tr>
                <th scope="row">{clipTweet(text, clip)}</th>
                <td>{username}</td>
                <td>{retweet_count}</td>
                <td>
                    <FontAwesomeIcon
                        icon={sent.icon}
                        className={`${sent.color} me-3`}
                    />
            ...
            <Card.Header>
                <Row className="align-items-center">
                    ...
                    <Col className="text-end">
                        <Button variant="secondary" size="sm" onClick={seeAll}>
                            See all
                        </Button>
                    </Col>
                </Row>
...
export const AllTweetsTable = (props) => {
